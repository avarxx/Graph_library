...
namespace graph
{
  ...
  void AdjacencyListGraph<VertexType, EdgeType>::addVertex(int id)
  {
    if (!this->hasVertex(id))
    {
      ...
      adjacencyList[id] = std::vector<int>(); // Инициализируем пустой список смежности
    }
  }
  ...
  void AdjacencyListGraph<VertexType, EdgeType>::addEdge(int source, int target)
  {
        ...
        !this->hasEdge(source, target))
    {
      ...
      adjacencyList[source].push_back(target); // Добавляем target в список смежности source
    }
  }
  ...
  template <typename VertexType, typename EdgeType> 
  bool AdjacencyListGraph<VertexType, EdgeType>::hasEdge(int source, int target) const 
  {
