...
/**
 * @brief Реализация алгоритма Флойда-Уоршелла для поиска кратчайших путей между всеми парами вершин.
...
std::vector<std::vector<WeightType>> FloydWarshall(const WeightedGraph<VertexType, WeightType>& graph) {
    const WeightType INF = std::numeric_limits<WeightType>::max();
    size_t n = graph.getVertices().size();
    std::vector<std::vector<WeightType>> distance(n, std::vector<WeightType>(n, INF));
