...
std::vector<WeightType> Dijkstra(WeightedGraph<VertexType, WeightType>& graph, const VertexType& start) {
    ...
    auto cmp = [](const std::pair<WeightType, VertexType>& left, const std::pair<WeightType, VertexType>& right) {
        return left.first > right.first;
    };
    std::priority_queue<std::pair<WeightType, VertexType>, std::vector<std::pair<WeightType, VertexType>>, decltype(cmp)> pq(cmp);
    pq.push({0, start});
